// simple-copy.mjs - Versión sencilla para copiar a una playlist existente o crear una nueva
...
async function main() {
  try {
    ...
    if (!sourceInput) {
      console.error('Usage: node simple-copy.mjs <sourcePlaylistUrlOrId> [targetPlaylistUrlOrId]');
      console.error('If targetPlaylistUrlOrId is provided, tracks will be copied to that existing playlist');
    ...
    if (targetPlaylistId) {
      ...
      try {
        ...
        console.log(`node simple-copy.mjs ${sourceInput} TU_PLAYLIST_ID_AQUI`);
        process.exit(1);
      }
    }
    ...
    console.log(`Adding ${uniqueTracks.length} tracks to the playlist...`);
    for (let i = 0; i < uniqueTracks.length; i += 100) {
      const batch = uniqueTracks.slice(i, i + 100);
      console.log(`Adding batch ${Math.floor(i/100) + 1}/${Math.ceil(uniqueTracks.length/100)}...`);
      await spotify.addTracksToPlaylist(targetId, batch);
    }
    
    console.log(`✅ Successfully copied ${uniqueTracks.length} tracks to "${targetName}" (${targetId})`);
